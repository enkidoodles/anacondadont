...
# SIDEBAR
	# Title
title = pyglet.resource.image('aTitle.png')
...
# BACKGROUNDS
bg = pyglet.resource.image("grass.png")
bg.width, bg.height = 600, 600
fence = pyglet.resource.image("electricalfence.png")
...
class Snake(pyglet.sprite.Sprite):
	...
	def reset(self):
		...
		self.vx, self.vy = math.cos(direction)*200, math.sin(direction)*200
...
class House(pyglet.sprite.Sprite):
	...
	def reset(self):
		...
		house_image = pyglet.image.SolidColorImagePattern((self.a[self.i],self.b[self.i],self.c[self.i],self.d))
		...
		self.x, self.y =  random.choice([100,200,300,400]), random.choice([100,200,300,400])
		
		while [self.x, self.y] in [[200,300],[300,200],[200,200],[300,300]]:
			self.x, self.y = random.choice([100,200,300,400]), random.choice([100,200,300,400])
...
class Elsa(pyglet.sprite.Sprite):
	...
	def __init__(self):
		...
		elsa_image.anchor_x, elsa_image.anchor_y = 0, 0
	...
	def reset(self):
		...
		while (house.x - 20 <= self.x <= house.x + 110) and (house.y - 20 <= self.y <= house.y + 110):
			self.x, self.y = random.randint(1,6) * 75, random.randint(1,6) * 75

class Poison(pyglet.sprite.Sprite):
	
	def __init__(self):
		
		self.type = random.randint(1,3)
		
		poison_image = pyglet.resource.image("poison" + str(self.type) + ".png")
		poison_image.width, poison_image.height = 30, 30
		poison_image.anchor_x, pyglet.anchor_y = 0,0
		
		pyglet.sprite.Sprite.__init__(self, poison_image)
		self.reset()
	...
	def reset(self):
		...
		while (house.x - 20 <= self.x <= house.x + 200) and (house.y - 20 <= self.y <= house.y + 200):
...
class PausedState():
	...
	def update(self,dt):
		...
		if keys[pyglet.window.key.SPACE]:
			...
			pyglet.clock.schedule_interval(states[0].bonus_creator, 5)
			pyglet.clock.schedule_interval(states[0].poison_creator, 5)
			states.pop()
			player.play()
...
class GameState():
	...
	def update(self, dt):
		...
		if keys[pyglet.window.key.P]:
			...
			pyglet.clock.unschedule(self.poison_creator)
	...
	def handle_collision(self,dt):
		...
		if (self.house.x <= self.snake.x + 10) and (self.snake.x - 10 <= self.house.x + 100) and (self.house.y <= self.snake.y + 10) and (self.snake.y - 10 <= self.house.y + 100):
			...
			if house.i == 0:
				...
				condition_label_1.text = "GAME OVER"
				condition_label_2.text = "YOU HIT A TREE"
				condition_label_3.text = "SCORE: " + "%01d" % self.snake.points 
		...
		for human in self.humans:
			...
			if dead == True:
				...
				if human.speed == 0:
					self.snake.points += 100
				elif human.speed == 150:
					self.snake.points += 200
				elif human.speed == 200:
					self.snake.points += 300
				else:
					self.snake.points += 500
				
				pointcounter.text = "%01d" % self.snake.points 
		
		for bonus in self.bonuses:
		
			if (bonus.x <= self.snake.x + 10) and (self.snake.x - 10 <= bonus.x + 30) and (bonus.y <= self.snake.y + 10) and (self.snake.y - 10 <= bonus.y + 30):
			
				self.consumed_bonuses.append(self.bonuses.index(bonus))
				
				for human in self.humans:
					human.freeze()
					human.vx_holder, human.vy_holder = 0,0
				
				pyglet.clock.unschedule(self.bonus_creator)
				pyglet.clock.unschedule(self.poison_creator)
				pyglet.clock.unschedule(self.human_creator)
				pyglet.clock.schedule_interval(self.let_it_go, 5)
				
				
		for poison in self.poisons:
		
			if (poison.x <= self.snake.x + 10) and (self.snake.x - 10 <= poison.x + 30) and (poison.y <= self.snake.y + 10) and (self.snake.y - 10 <= poison.y + 30):
				
				if poison.type in [1,2]:
					
					self.consumed_poisons.append(self.poisons.index(poison))
					poison.x, poison.y = -900, -900
					self.snake.points -= 200
					pointcounter.text = "%01d" % self.snake.points
				
				else:
					condition_label_1.text = "GAME OVER"
					condition_label_2.text = "YOU ATE RADIOACTIVE WASTE"
					condition_label_3.text = "SCORE: " + "%01d" % self.snake.points 
	...
	def let_it_go(self,dt):
		...
		if self.counter == 1:
			...
			pyglet.clock.schedule_interval(self.bonus_creator, 5)
			pyglet.clock.schedule_interval(self.human_creator, 1.5)
			pyglet.clock.schedule_interval(self.poison_creator, 5)
	...
	def human_creator(self,dt):
		...
		if self.population > 10:
			...
			if len(self.humans) <= 4:
				...
				self.choice_of_speed = random.choice([0,150,200])
			elif len(self.humans) <= 13:
				self.choice_of_speed = 200
			elif len(self.humans) <= 16:
				self.choice_of_speed = random.choice([150,200,200])
			else:
				self.choice_of_speed = random.choice([0,150,200,200,300,300,300,300])
...
def on_draw():
	...
	for i in range(len(states[0].poisons)):
		if i not in states[0].consumed_poisons:
			states[0].poisons[i].draw()
