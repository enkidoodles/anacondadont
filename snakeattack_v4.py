...
# SIDEBAR
	# Title
title = pyglet.resource.image('temptitle.jpg')
...
snake_buns = pyglet.resource.image('buns.jpg')
...
x = highscorefile.readline()
hscounter = pyglet.text.Label(x, font_name='Arial', font_size=17, x=700, y=35, anchor_x='center', anchor_y='center', batch=labels)
highscorefile.close()

trophy = pyglet.resource.image('hs.jpg')
	...
	def handle_collision(self,dt):
		...
		for bonus in self.bonuses:
			...
			if (bonus.x - 15 <= self.snake.x + 10) and (self.snake.x - 10 <= bonus.x + 15) and (bonus.y - 15 <= self.snake.y + 10) and (self.snake.y - 10 <= bonus.y + 12):
				...
				pyglet.clock.unschedule(self.bonus_creator)
				pyglet.clock.unschedule(self.human_creator)
		humancounter.text = "%01d" % self.count
